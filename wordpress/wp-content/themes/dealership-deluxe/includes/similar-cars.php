				...
				foreach ($terms as $term) {
					...
					if(have_posts()){ 
						while ( have_posts() ) : the_post(); $do_not_duplicate[] = $post->ID; 
							require_once(TEMPLATEPATH."/functions/var/default-box-one.php");
							...
							<div>
								...
								<div class="meta-style">
									...
									<?php	 if ( $fields['miles']){ echo $fields['miles'].' '.$options['milestext'];} elseif ($fields['miles'] == '0' ){ echo _e('0','language').' '.$options['milestext'];} else {echo '';}  ?>
								</div>
								<p class="vehicle-bbprice">
									<span class='strike'>
										<?php  if (is_numeric( $fields['blackbookprice'])){ echo $symbols['currency']; echo number_format($fields['blackbookprice']);} else {  echo $fields['blackbookprice']; } ?>
									</span>
								</p>

								<div class="price-style">
									<?php  if ( $fields['price']){ echo $symbols['currency']; echo number_format($fields['price']);}else {  echo ''; } ?>
								</div>
